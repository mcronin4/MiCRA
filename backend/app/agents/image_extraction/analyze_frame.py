...
# Globals for the Places365 model files (paths relative to this module)
_MODULE_DIR = os.path.dirname(os.path.abspath(__file__))
PLACES_CATEGORIES_FILE = os.path.join(_MODULE_DIR, 'categories_places365.txt')
PLACES_WEIGHTS_FILE = os.path.join(_MODULE_DIR, 'resnet18_places365.pth.tar')
...
def analyze_emotion(frame_bgr, detector_backend='opencv', min_face_size=40, min_confidence=0.0):
    """
    Get facial emotion analysis for a single frame.
    
    Args:
        frame_bgr: OpenCV BGR image
        detector_backend: 'opencv' is most compatible; 'retinaface' has Keras 3 issues
        min_face_size: Minimum face width/height in pixels to be considered valid
    ...
    try:
        ...
        # Check if a face region was returned
        region = main_face.get("region", {})
        if not region:
            return {"face_detected": False, "dominant_emotion": None, "probabilities": {}}
        
        # Validate face size (reject tiny "faces" that are likely false positives)
        face_w = region.get("w", 0)
        face_h = region.get("h", 0)
        if face_w < min_face_size or face_h < min_face_size:
            return {"face_detected": False, "dominant_emotion": None, "probabilities": {}}
        
        # Check confidence if available (DeepFace >= 0.0.79)
        confidence = main_face.get("face_confidence", 1.0)
        ...
        return {
            ...
            "face_confidence": float(confidence) if confidence else None
        }
