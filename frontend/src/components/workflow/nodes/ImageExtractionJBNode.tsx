...
import {
  extractKeyframesJBFromFile,
  extractKeyframesJBFromUrl,
  ExtractedImageJB,
} from "@/lib/fastapi/image-extraction-jb";
import { NodeConfig } from "@/types/workflow";
import { Upload, Link, X, Image as ImageIcon, Settings2 } from "lucide-react";
import Image from "next/image";

type SourceType = "file" | "url";

const config: NodeConfig = {
  type: "image-extraction-jb",
  label: "Image Extraction (JB)",
  description: "Dense frame extraction with smart deduplication",
  inputs: [
...
export function ImageExtractionJBNode({ id }: NodeProps) {
  ...
  const initialImages = Array.isArray(nodeOutputs.images)
    ? (nodeOutputs.images as ExtractedImageJB[])
    : [];
  ...
  // JB Edition specific config
  const [frameInterval, setFrameInterval] = useState<number>(
    typeof node?.inputs?.frame_interval === "number" ? node.inputs.frame_interval : 3
  );
  const [maxFinalFrames, setMaxFinalFrames] = useState<number>(
    typeof node?.inputs?.max_final_frames === "number" ? node.inputs.max_final_frames : 25
  );
  const [temporalBuckets, setTemporalBuckets] = useState<number>(
    typeof node?.inputs?.temporal_buckets === "number" ? node.inputs.temporal_buckets : 5
  );
  ...
  const [selectedImages, setSelectedImages] =
    useState<ExtractedImageJB[]>(initialImages);
  ...
  const scoredImages = useMemo(() => {
    ...
    return selectedImages
      .map((image) => ({
        ...image,
        score: scoreLookup.get(image.filename) ?? image.quality_score ?? null,
      }))
      .sort((a, b) => {
  ...
  useEffect(() => {
    ...
    if (
      ...
      node.inputs.frame_interval !== frameInterval ||
      node.inputs.max_final_frames !== maxFinalFrames ||
      node.inputs.temporal_buckets !== temporalBuckets
    ) {
      updateNode(id, {
        inputs: {
          ...node.inputs,
          ...
          max_final_frames: maxFinalFrames,
          temporal_buckets: temporalBuckets,
        },
      });
    }
  }, [sourceType, videoUrl, fileName, frameInterval, maxFinalFrames, temporalBuckets, id, updateNode, node]);
  ...
  const pushImagesToBucket = (images: ExtractedImageJB[]) => {
  ...
  const handleExecute = async () => {
    ...
    try {
      if (sourceType === "url") {
        ...
        const response = await extractKeyframesJBFromUrl(videoUrl.trim(), options);
        if (!response.success) {
        ...
        const response = await extractKeyframesJBFromFile(selectedFile, options);
        if (!response.success) {
          throw new Error(response.error || "Image extraction failed");
        }
        const nextImages = response.selected_images || [];
      ...
      <div className="space-y-4">
        {showManualInputs && (
          <>
            ...
            <button
              ...
              className="nodrag flex items-center gap-1.5 text-[11px] text-slate-500 hover:text-violet-600 transition-colors"
            >
              <Settings2 size={12} />
              ...
              <div className="space-y-3 p-3 bg-slate-50 rounded-xl border border-slate-200">
                <div className="space-y-1">
                  <div className="flex justify-between text-[11px]">
                    <span className="text-slate-600 font-medium">Frame Interval</span>
                    <span className="text-slate-500">every {frameInterval} frames</span>
                  </div>
                  <input
                    type="range"
                    min={1}
                    ...
                    disabled={isRunning}
                    className="nodrag w-full h-1.5 bg-slate-200 rounded-full appearance-none cursor-pointer accent-violet-600"
                  />
                  <p className="text-[10px] text-slate-400">Lower = more samples (slower)</p>
                </div>

                <div className="space-y-1">
                  <div className="flex justify-between text-[11px]">
                    <span className="text-slate-600 font-medium">Max Frames</span>
                    <span className="text-slate-500">{maxFinalFrames}</span>
                  </div>
                  <input
                    type="range"
                    min={5}
                    max={50}
                    value={maxFinalFrames}
                    onChange={(e) => setMaxFinalFrames(Number(e.target.value))}
                    disabled={isRunning}
                    className="nodrag w-full h-1.5 bg-slate-200 rounded-full appearance-none cursor-pointer accent-violet-600"
                  />
                  <p className="text-[10px] text-slate-400">Maximum frames to return</p>
                </div>

                <div className="space-y-1">
                  <div className="flex justify-between text-[11px]">
                    <span className="text-slate-600 font-medium">Temporal Buckets</span>
                    <span className="text-slate-500">{temporalBuckets}</span>
                  </div>
                  <input
                    type="range"
                    min={2}
                    max={10}
                    value={temporalBuckets}
                    onChange={(e) => setTemporalBuckets(Number(e.target.value))}
                    disabled={isRunning}
                    className="nodrag w-full h-1.5 bg-slate-200 rounded-full appearance-none cursor-pointer accent-violet-600"
                  />
                  <p className="text-[10px] text-slate-400">Ensures coverage across video timeline</p>
                </div>
              </div>
            )}
          </>
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <input
                ...
                disabled={isRunning}
                className="nodrag w-full pl-9 pr-3 py-2.5 text-sm border border-slate-200 rounded-xl bg-slate-50 focus:bg-white focus:ring-2 focus:ring-violet-500/20 focus:border-violet-400 transition-all"
              />
            </div>
          </div>
        )}
              ...
              <div className="flex flex-col items-center justify-center text-center py-4">
                ...
                <p
                  className={`text-sm font-medium ${
                    isDragging ? "text-violet-700" : "text-slate-700"
                  }`}
                >
        ...
        {stats && (
          <div className="flex flex-wrap gap-2 text-[11px] text-slate-500">
            {"total_frames_sampled" in stats && (
              <span className="px-2.5 py-1 rounded-full border border-violet-200 bg-violet-50 text-violet-700">
                Sampled: {stats.total_frames_sampled}
              </span>
            )}
            {"after_deduplication" in stats && (
              <span className="px-2.5 py-1 rounded-full border border-slate-200 bg-white">
                After Dedup: {stats.after_deduplication}
              </span>
            )}
            {"final_selected" in stats && (
              <span className="px-2.5 py-1 rounded-full border border-green-200 bg-green-50 text-green-700">
