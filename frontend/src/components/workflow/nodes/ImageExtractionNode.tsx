...
import { extractKeyframesFromFile, extractKeyframesFromUrl, ExtractedImage } from '@/lib/fastapi/image-extraction'
...
export function ImageExtractionNode({ id }: NodeProps) {
  ...
  const nodeOutputs = node?.outputs && typeof node.outputs === 'object'
    ? node.outputs as Record<string, unknown>
    : {}

  const initialSourceType: SourceType = node?.inputs?.source_type === 'url' ? 'url' : 'file'
  const initialUrl = typeof node?.inputs?.url === 'string' ? node.inputs.url : ''
  const initialFileName = typeof node?.inputs?.file_name === 'string' ? node.inputs.file_name : ''
  const initialImages = Array.isArray(nodeOutputs.images) ? nodeOutputs.images as ExtractedImage[] : []
  const initialMetadata = Array.isArray(nodeOutputs.metadata) ? nodeOutputs.metadata as Record<string, unknown>[] : []
  const initialStats = nodeOutputs.stats && typeof nodeOutputs.stats === 'object' && !Array.isArray(nodeOutputs.stats)
  ...
  const [selectedImages, setSelectedImages] = useState<ExtractedImage[]>(initialImages)
  ...
  const scoredImages = useMemo(() => {
    ...
    metadata.forEach((frame) => {
      const rawPath = typeof frame.selected_path === 'string'
        ? frame.selected_path
        : typeof frame.frame_path === 'string'
          ? frame.frame_path
          : ''
      const filename = rawPath ? rawPath.split(/[\\/]/).pop() : ''
      const score = typeof frame.quality_score === 'number' ? frame.quality_score : null
  ...
  const handleExecute = async () => {
    ...
    try {
      if (sourceType === 'url') {
        ...
        setSelectedImages(response.selected_images || [])
        setMetadata(response.selected_frames || [])
        setStats(response.stats || null)
        updateNode(id, {
          status: 'completed',
          outputs: {
            images: response.selected_images || [],
    ...
    <WorkflowNodeWrapper nodeId={id} config={config} onExecute={handleExecute}>
      <div className="space-y-3">
        <div className="flex gap-2">
          <button
            ...
            className={`nodrag flex-1 px-3 py-1.5 text-xs font-medium rounded-md transition-colors ${
              sourceType === 'file'
        ...
        {sourceType === 'url' && (
          <div>
            <label className="text-xs text-gray-600 block mb-1">YouTube URL</label>
            <div className="relative">
              <Link size={14} className="absolute left-2 top-1/2 -translate-y-1/2 text-gray-400" />
              <input
                type="url"
                value={videoUrl}
                ...
                disabled={isRunning}
                className="nodrag w-full pl-7 pr-2 py-1.5 text-sm border rounded"
              />
            </div>
          </div>
        )}
        ...
        {sourceType === 'file' && (
          <div>
            <label className="text-xs text-gray-600 block mb-1">Video File</label>
            <input
              ref={fileInputRef}
              type="file"
            ...
            <div
              ...
              className={`
                ...
                ${isDragging ? 'border-purple-500 bg-purple-50' : 'border-gray-300 bg-gray-50 hover:border-gray-400'}
                ${isRunning ? 'opacity-60 cursor-not-allowed' : ''}
              `}
            >
              <div className="flex flex-col items-center justify-center text-center py-4">
                <div className={`mb-1 ${isDragging ? 'text-purple-600' : 'text-gray-400'}`}>
                  <Upload size={24} />
                </div>
                ...
                <p className="text-[10px] text-gray-400 mt-1">MP4 only</p>
              </div>
            </div>
            {fileName && (
              <button
                ...
                onClick={clearFile}
                disabled={isRunning}
                className="nodrag mt-2 inline-flex items-center gap-1 text-xs text-red-600 hover:text-red-700 disabled:opacity-60"
              >
                <X size={12} />
                Remove file
              </button>
        ...
        {hasResults && (
          <div
            className="nodrag border rounded-lg bg-gray-50 p-3 cursor-pointer transition-shadow hover:shadow-md"
          ...
          >
            <div className="flex items-center justify-between text-xs text-gray-600 mb-2">
              <span className="font-medium">Selected frames</span>
            ...
            <div className="flex gap-2">
              ...
              {topImages.length === 0 && (
                <div className="text-xs text-gray-500">No images selected yet.</div>
              )}
            </div>
      ...
      {showGallery && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          />
          <div
            className="nodrag relative w-[95vw] max-w-6xl max-h-[92vh] bg-white rounded-xl shadow-2xl overflow-hidden"
            onClick={(event) => event.stopPropagation()}
          >
            <div className="flex items-center justify-between px-4 py-3 border-b bg-white">
              <div>
                <h4 className="text-sm font-semibold text-gray-900">Selected frames</h4>
                <p className="text-xs text-gray-500">Click any image to download</p>
              </div>
              <button
                type="button"
                className="nodrag p-1 rounded-full hover:bg-gray-100"
                onClick={() => setShowGallery(false)}
            ...
            <div className="p-4 overflow-y-auto max-h-[calc(92vh-60px)]">
              <div
              ...
              >
                {scoredImages.map((img) => (
                  <a
                    ...
                    className="nodrag group relative rounded-lg border bg-white overflow-hidden transition-shadow hover:shadow-md"
                  ...
                  >
                    ...
                    <div className="p-2 text-[10px] text-gray-600 truncate">
                      {img.filename}
                    </div>
