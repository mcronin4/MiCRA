...
import { isVideoValue } from '@/lib/preview-utils'
...
export function DroppableSlotContainer({
  ...
  if (stale) {
    return (
      <button
        ref={setNodeRef}
        onClick={onClick}
        className={`w-full text-left rounded-lg border-2 border-dashed border-amber-300 bg-amber-50 p-3 transition-all hover:border-amber-400 ${isActive ? 'ring-2 ring-amber-300' : ''
  ...
  if (!hasValue) {
    ...
    return (
      <button
        ref={setNodeRef}
        onClick={onClick}
        className={`w-full text-left rounded-lg border-2 border-dashed p-3 transition-all ${showDropHighlight
  ...
  return (
    <button
      ...
      type="button"
      className={`w-full text-left rounded-lg border p-3 transition-all ${showDropHighlight
    ...
    >
      {slot.acceptsTypes.includes('image') || slot.acceptsTypes.includes('video') ? (
        slot.acceptsTypes.includes('video') && isVideoValue(value) ? (
